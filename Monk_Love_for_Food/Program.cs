...
namespace Monk_Love_for_Food
{
    class Program
    {
        static void Main(string[] args)
        {
            Stack<int> food_pile = new Stack<int>();

            int query_count = Int32.Parse(Console.ReadLine());
            while (query_count > 0)
            {
                string[] read_query = Console.ReadLine().Split(' ');
                if (Int32.Parse(read_query[0]) == 1)
                {
                    if (food_pile.Count == 0)
                        Console.WriteLine("No Food");
                    else
                        Console.WriteLine(food_pile.Pop());
                }
                else
                    food_pile.Push(Int32.Parse(read_query[1]));
